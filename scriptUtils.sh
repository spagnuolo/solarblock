...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"public-channel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c public-channel -s couchdb -i 2.0.0"
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from orgNetzbetreiber and orgHaushaltA"
