...
type Asset struct {
	...
	ID    string `json:"energyNumber"`
  SellDateTime string `json:"sellDateTime"`
  ExpiredDateTime string `json:"expiredDateTime"`
	Seller string `json:"seller"`

}

// AssetPrivateDetails describes details that are private to sellers
...
// that can be read by both organizations. The appraisal value is stored in the sellers org specific collection.
func (s *SmartContract) CreateAsset(ctx contractapi.TransactionContextInterface) error {
	...
	if len(assetInput.ID) == 0 {
		return fmt.Errorf("energyNumber field must be a non-empty string")
	}
	if len(assetInput.SellDateTime) == 0 {
		return fmt.Errorf("sellDateTime field must be a non-empty string")
	}
	if len(assetInput.ExpiredDateTime) == 0 {
		return fmt.Errorf("expiredDateTime field must be a non-empty string")
	}
	if assetInput.FaceValue <= 0 {
		return fmt.Errorf("faceValue field must be a positive integer")
	}
	...
	// Make submitting client the seller
	asset := Asset{
		Type:  assetInput.Type,
		ID:    assetInput.ID,
		SellDateTime: assetInput.SellDateTime,
		ExpiredDateTime:  assetInput.ExpiredDateTime,
		Seller: clientID,
	}
	...
	log.Printf("CreateAsset Put: collection %v, ID %v, seller %v", assetCollection, assetInput.ID, clientID)
	...
	assetPrivateDetails := AssetPrivateDetails{
		ID:             assetInput.ID,
		FaceValue: assetInput.FaceValue,
	}
	...
	// Put asset appraised value into sellers org specific private data collection
...
// TransferAsset transfers the asset to the new seller by setting a new seller ID
func (s *SmartContract) TransferAsset(ctx contractapi.TransactionContextInterface) error {
	...
	transientTransferJSON, ok := transientMap["asset_seller"]
	if !ok {
		return fmt.Errorf("asset seller not found in the transient map")
	}
	...
	// Verify transfer details and transfer seller
	err = s.verifyAgreement(ctx, assetTransferInput.ID, asset.Seller, assetTransferInput.BuyerMSP)
	if err != nil {
	...
	// Transfer asset in private data collection to new seller
	asset.Seller = transferAgreement.BuyerID
	...
	// Get collection name for this organization
	sellersCollection, err := getCollectionName(ctx)
	if err != nil {
		return fmt.Errorf("failed to infer private collection name for the org: %v", err)
	}

	// Delete the asset appraised value from this organization's private data collection
	err = ctx.GetStub().DelPrivateData(sellersCollection, assetTransferInput.ID)
	if err != nil {
		return err
	}
...
// that the transfer is being initiated by the seller and that the buyer has agreed
// to the same appraisal value as the seller
func (s *SmartContract) verifyAgreement(ctx contractapi.TransactionContextInterface, energyNumber string, seller string, buyerMSP string) error {

	// Check 1: verify that the transfer is being initiatied by the seller
	...
	collectionSeller, err := getCollectionName(ctx) // get seller collection from caller identity
	if err != nil {
		return fmt.Errorf("failed to infer private collection name for the org: %v", err)
	}
	...
	// Get hash of sellers agreed to value
	sellerFaceValueHash, err := ctx.GetStub().GetPrivateDataHash(collectionSeller, energyNumber)
	if err != nil {
		return fmt.Errorf("failed to get hash of appraised value from sellers collection %v: %v", collectionSeller, err)
	}
	if sellerFaceValueHash == nil {
		return fmt.Errorf("hash of appraised value for %v does not exist in collection %v", energyNumber, collectionSeller)
	}

	// Get hash of buyers agreed to value
	buyerFaceValueHash, err := ctx.GetStub().GetPrivateDataHash(collectionBuyer, energyNumber)
	if err != nil {
		return fmt.Errorf("failed to get hash of appraised value from buyer collection %v: %v", collectionBuyer, err)
	}
	if buyerFaceValueHash == nil {
		return fmt.Errorf("hash of appraised value for %v does not exist in collection %v. AgreeToTransfer must be called by the buyer first", energyNumber, collectionBuyer)
	}

	// Verify that the two hashes match
	if !bytes.Equal(sellerFaceValueHash, buyerFaceValueHash) {
		return fmt.Errorf("hash for appraised value for seller %x does not value for seller %x", sellerFaceValueHash, buyerFaceValueHash)
	}

	return nil
}

// DeleteAsset can be used by the seller of the asset to delete the asset
func (s *SmartContract) DeleteAsset(ctx contractapi.TransactionContextInterface) error {
	...
	sellerCollection, err := getCollectionName(ctx) // Get sellers collection
	if err != nil {
		return fmt.Errorf("failed to infer private collection name for the org: %v", err)
	}

	//check the asset is in the caller org's private collection
	valAsbytes, err = ctx.GetStub().GetPrivateData(sellerCollection, assetDeleteInput.ID)
	if err != nil {
		return fmt.Errorf("failed to read asset from seller's Collection: %v", err)
	}
	if valAsbytes == nil {
		return fmt.Errorf("asset not found in seller's private Collection %v: %v", sellerCollection, assetDeleteInput.ID)
	}
	...
	err = ctx.GetStub().DelPrivateData(sellerCollection, assetDeleteInput.ID)
	if err != nil {
		return err
	}
