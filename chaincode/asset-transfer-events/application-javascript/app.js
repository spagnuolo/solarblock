...
//         ===> from directory test-network
//         ./network.sh deployCC -ccn events -ccl javascript -ccep "OR('OrgNetzbetreiberMSP.peer','OrgHaushaltAMSP.peer')"
//
		...
		try {
			...
			try {
				...
				if (deleteError.toString().includes('ENDORSEMENT_POLICY_FAILURE')) {
					console.log(`${RED}Be sure that chaincode was deployed with the endorsement policy "OR('OrgNetzbetreiberMSP.peer','OrgHaushaltAMSP.peer')"${RESET}`)
				}
			}
			try {
				// R E A D
